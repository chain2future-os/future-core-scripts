...
def transferReg():
    ...
    #retry(args.clfuture+'system empoweruser '+user+' '+dest+' '+userPK+' '+userPK+' 1  -p '+user+'@active');
    sleep(5)
    ...
    retry(args.clfuture + 'system regproducer ' + user +' '+minerPK+' '+bls_key+' '+user+' https://'+user+'.com '+dest+' -p futureio@active -u')
...
def setSchedule():
    ...
    if args.schedPeriod is None or args.confirmPeriod is None :
        ...
        return
    retry(args.clfuture + 'push action futureio setsched \'{"is_enabled": "1", "sched_period": "'+args.schedPeriod+'", "expire_time": "'+args.confirmPeriod+'"}\' -p futureio@active')


def closeSchedule():
    print "close schedule"
    retry(args.clfuture + 'push action futureio setsched \'{"is_enabled": "0", "sched_period": "600", "expire_time": "5"}\' -p futureio@active')

def serachTable(contract,scope,table):
    return "get table "+contract+" "+scope+" "+table

def showSubchains():
    retry(args.clfuture+serachTable("futureio","futureio","subchains"));
