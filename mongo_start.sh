...
AUTH_PARA="-u $USER -p $PASS --gzip --authenticationDatabase future"
DB="$IP/future"
...
do
   ...
   if [ $start_blknum -eq $end_blknum ] ; then
       echo "restore $i"
       $MONGORESTORE $AUTH_PARA -d future --excludeCollection account_controls --excludeCollection accounts --excludeCollection pub_keys $OUT_DIR/$i/future
       if [ "$?" -ne 0 ] ; then
...
$MONGORESTORE $AUTH_PARA -d future -c account_controls $OUT_DIR/$extra_dir/future/account_controls.bson.gz 
$MONGORESTORE $AUTH_PARA -d future -c accounts $OUT_DIR/$extra_dir/future/accounts.bson.gz 
$MONGORESTORE $AUTH_PARA -d future -c pub_keys $OUT_DIR/$extra_dir/future/pub_keys.bson.gz 
echo $[end_blknum-1]
