...
var logger = require("../config/logConfig").getLogger("NodFuture");
...
NodFuture.statusCheckTime = 500;

NodFuture.configFilePath = utils.formatHomePath("~/.local/share/futureio/nodfuture/config/config.ini");

NodFuture.biosGenesisPath = utils.formatHomePath("/root/workspace/future-core-scripts/bios-new-chain.py");

NodFuture.checkConfigInfo = function(updateArr) {
    try {
        let ready = true;
 ...
 * @param updateArr
 */
NodFuture.updateConfigInfo = function(updateArr) {
    try {
 ...
 * @returns {boolean}
 */
NodFuture.addConfigInfo = function(updateArr) {
    try {
        let updateFlag = false;
        var iniFile = new IniFile(this.configFilePath, Constants.encodingConstants.UTF8);
        if (updateArr.length > 0) {
 ...
 * @returns {boolean}
 */
NodFuture.updateMongoStartNum = function(num) {
    try {
        var iniFile = new IniFile(this.configFilePath, Constants.encodingConstants.UTF8);
 ...
 * @returns {boolean}
 */
NodFuture.updateConfig = function (seedIp,privateIpFlag,subchainHttpEndpoint,genesisTime,genesisPK,monitorServcer,chainPeerInfo,chainName) {
    try {
        var iniFile = new IniFile(this.configFilePath, Constants.encodingConstants.UTF8);
        ...
        logger.error("update nodfuture config file error: ", e)
        return false;
    }
}
 ...
 */
NodFuture.stop = async function (totalTime,port) {
    let res = await ShellCmd.execCmd(Constants.cmdConstants.KILL_NODFUTURE);
 ...
 * @returns {Promise<boolean>}
 */
NodFuture.execGenesisBios = async function ( genesisBiosCmd ) {
    return await ShellCmd.execCmd( genesisBiosCmd );
}
 ...
 * @returns {Promise<boolean>}
 */
NodFuture.start = async function (totalTime,nodPath,wssinfo,local,port) {

    var shPath= path.join(__dirname, "../../tool/_runfuture.sh");
    if (local == false) {
        shPath = Constants.cmdConstants.START_NODFUTURE_FILE;
    }
...
//检查是否已启动
NodFuture.checkAlive = async function (port) {
...
//删除数据
NodFuture.removeData = async function () {
    try {
 ...
 */
NodFuture.updateExeFile = async function (localpath,targetpath,hashFile,port) {
    let result = false;
    try {
...
NodFuture.getNewestLog = function(logDir,callback) {
    try {
