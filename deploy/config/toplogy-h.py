...
def insert_leader_sk(fname):
	...
	newcontent = "my-account-as-committee = %s\nmy-sk-as-committee = %s\nmy-sk-as-account = %s\nmy-bls-sk = %s\n" % ("genesis", \
																													 ...
																													 "5KJA4NykBcw48bzBTYk54LpmPQSLAFmjKBaoVmMASJVFCawM4NH", \
...
def insert_non_producing(fname):
    ...
    newcontent += "http-validate-host = false\n"
	print(newcontent)
	index_line = content.index("#insert_if_producing-node\n")
	content.insert(index_line+1, newcontent)
	content.insert(index_line+2, "plugin = futureio::chain_api_plugin\n")
...
def insert_mongo_config(fname):
	...
	content.insert(index_line+3, "mongodb-uri = mongodb://root:Uranus@127.0.0.1:27017/future\n");
	content.insert(index_line+4, "plugin = futureio::mongo_db_plugin\n");
	writefile(fname,content)
