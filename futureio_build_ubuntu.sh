	...
	case "${OS_NAME}" in
		"Linux Mint")
		   if [ "${OS_MAJ}" -lt 18 ]; then
			   printf "\\tYou must be running Linux Mint 18.x or higher to install FUTUREIO.\\n"
			   printf "\\tExiting now.\\n"
			   exit 1
		   fi
		;;
		"Ubuntu")
			if [ "${OS_MAJ}" -lt 16 ]; then
				printf "\\tYou must be running Ubuntu 16.04.x or higher to install FUTUREIO.\\n"
				printf "\\tExiting now.\\n"
				exit 1
			fi
		;;
		"Debian")
			if [ $OS_MAJ -lt 10 ]; then
				printf "\tYou must be running Debian 10 to install FUTUREIO, and resolve missing dependencies from unstable (sid).\n"
				printf "\tExiting now.\n"
				exit 1
		fi
		;;
	...
	if [ "${DISK_AVAIL%.*}" -lt "${DISK_MIN}" ]; then
		printf "\\tYou must have at least %sGB of available storage to install FUTUREIO.\\n" "${DISK_MIN}"
		printf "\\tExiting now.\\n"
		exit 1
	fi
	...
	if [ "${COUNT}" -gt 1 ]; then
		printf "\\n\\tThe following dependencies are required to install FUTUREIO.\\n"
		printf "\\n\\t${DISPLAY}\\n\\n"
    ...
    echo ${b:7:6}
    sed -i -e "s/futureio::abi.*/futureio::abi\/1.0\/${b:7:6}\",/g" $BUILD_DIR/../contracts/futureio.system/futureio.system.abi
