...
def sendEmail(msg,successFlag):
    ...
    :return:
    """
    receiver = "yanhuichao@future.io"#"#,suyu@future.io"
...
class RewardsTest(object):
    ...
    def GetProducerList( self ):
        ...
        for chain_name,url in testNetsideHttp.items():
            chainInfoJson = json.loads(requests.get(testNetmasterHttp["futureio"]+"/v1/chain/get_table_records",data = json.dumps({"code":"futureio","scope":"futureio","table":"chains","table_key":chain_name,"json":"true"})).text)

            producerInfoJson = json.loads(requests.get(testNetmasterHttp["futureio"]+"/v1/chain/get_producers",data = json.dumps({"all_chain":"false","chain_name":chain_name,"json":"true"})).text)
    ...
    def chainSnapshotSub( self ):
        ...
        for key,info in masterChainBlock.items():
            ...
            self.sendMsgInfo += "    矿工:%s ,所在链:%s,出块数量:%d  待发奖励:%.4f FGAS " %(key,info["chain_name"], info["block_num"], info["unpaid_balance"]/10000)
    ...
    def getMongoBlockList( self ):
        for chain_name,url in testNetsideHttp.items():
            ...
            for info in mongoList:
                ...
                if info["block"]["proposer"] == "futio.empty":
