...
def createmutiaccounts():
    for a in sidechainacc:
        if args.deleysec == 0:
            ...
            retry(args.clfuture + 'create account -u futureio ' + a + ' ' + args.initacc_pk+" --delay-sec "+str(args.deleysec))
    sleep(10)
    print "sidechain";
    print sidechainacc1;
    if args.subchainNum >= 1:
        for a in sidechainacc1:
            if args.deleysec == 0:
                ...
                retry(args.clfuture + ' system empoweruser %s %s %s %s 1 --delay-sec %s -p %s@active' %(a,"11",args.initacc_pk,args.initacc_pk,str(args.deleysec),"futureio"))
    if args.subchainNum >= 2:
        for a in sidechainacc2:
            if args.deleysec == 0:
                ...
                retry(args.clfuture + ' system empoweruser %s %s %s %s 1 --delay-sec %s -p %s@active' %(a,"12",args.initacc_pk,args.initacc_pk,str(args.deleysec),"futureio"))
    if args.subchainNum >= 3:
        for a in sidechainacc3:
            if args.deleysec == 0:
                ...
                retry(args.clfuture + ' system empoweruser %s %s %s %s 1 --delay-sec %s -p %s@active' %(a,"13",args.initacc_pk,args.initacc_pk,str(args.deleysec),"futureio"))


def initTransfer():
    for a in sidechainacc:
        retry(args.clfuture + 'transfer futureio %s "100 FGAS" "futureio"' %(a))

def transerMasterToSubchain():
    if args.subchainNum >= 1:
        for a in sidechainacc1:
            retry(args.clfuture + 'transfer %s futio.bank "10 FGAS" "11"' %(a));
    if args.subchainNum >= 2:
        for a in sidechainacc2:
            retry(args.clfuture + 'transfer %s futio.bank "10 FGAS" "12"' %(a));
    if args.subchainNum >= 3:
        for a in sidechainacc3:
            retry(args.clfuture + 'transfer %s futio.bank "10 FGAS" "13"' %(a));

def transerSubToMaster():
    # if args.subchainNum == 1:
    #     for a in sidechainacc1:
    #         retry(args.clfuture + 'transfer %s futio.bank "10 FGAS" "futureio"' %(a));
    # if args.subchainNum == 2:
    #     for a in sidechainacc2:
    #         retry(args.clfuture + 'transfer %s futio.bank "10 FGAS" "futureio"' %(a));
    # if args.subchainNum == 3:
    ...
    for i in range(0,100):
        for a in sidechainacc4:
            retry(args.clfuture  + ' -u http://172.16.10.9:8888 transfer %s futio.bank "1 FGAS" "22" -f ' %(a));
            sleep(3)
