...
parser.add_argument('-httpAlias', '--httpAlias', type=str, help="set http alias for nofuture")
...
parser.add_argument('-prodp', '--prodprefix', type=str, help="set producer name prefix")
parser.add_argument('--trx_cipher', metavar='', help="which cipher to use secp256k1/sm2", type=str, default="secp256k1", dest="trx_cipher")
parser.add_argument('--consensus_cipher', metavar='', help="which cipher to use ed25519/sm2", type=str, default="secp256k1", dest="consensus_cipher")
...
def load_parameters():
    ...
    if args.masterchain:
        ...
        for a in accounts[1:]:
            if args.prodprefix:
                adjustaccounts.append(args.prodprefix + a)
            else:
    ...
    if args.consensus_cipher == "sm2":
        allPriKeys = consensus_sm2_sk_list
        allPubKeys = consensus_sm2_pk_list
    else :
        allPriKeys = sk_list
        allPubKeys = pk_list
...
def write_config_file():
    ...
    for con in hosts[dockerinfo]:
        ...
        #update_futuremng_config(fname, hosts[dockerinfo])
...
# update futuremng  config
def update_futuremng_config(fname, ipList):
    content = readfile(fname)
    newcontent = "";
    ...
    index_line = content.index("#futuremng_subchainHttpEndpoint\n")
    content.insert(index_line + 1, newcontent)
    writefile(fname, content)
...
def insert_chain_name_config(fname):
    content = readfile(fname)
    newcontent = "chain-name=futureio\n";
    if args.subchain:
...
def insert_non_producing(fname):
    ...
    newcontent += "plugin = futureio::chain_api_plugin\n"
...
def insertPeerKey(fname):
    ...
    if args.trx_cipher == "sm2":
        newcontent = "peer-key = %s\n" % (trx_account_sm_pk_list[0])
        newcontent1 = "peer-key = %s\n" % (trx_account_sm_pk_list[6])
        newcontent2 = "peer-key = %s\n" % (trx_account_sm_pk_list[7])
    else :
        newcontent = "peer-key = %s\n" % (account_pk_list[0])
        newcontent1 = "peer-key = %s\n" % (account_pk_list[6])
        newcontent2 = "peer-key = %s\n" % (account_pk_list[7])
...
def insert_keys(fname, index_key):
    content = readfile(fname)
    account_sk = account_sk_list[index_key]
    if args.trx_cipher == "sm2":
        account_sk = trx_account_sm_sk_list[index_key]

    newcontent = "my-account-as-committee = %s\nmy-sk-as-committee = %s\nmy-sk-as-account = %s\nmy-bls-sk = %s\ngenesis-pk = %s\n" % \
                 (allAccounts[index_key], allPriKeys[index_key], account_sk, bls_sk_list[index_key], allPubKeys[0])
