...
class RegDelProd(unittest.TestCase):
    ...
    def regProducer( self ):
        cmd_exec(Config.clfuture_path + '''  system regproducer %s %s %s %s "" "futureio" -u ''' % ( self._name, self._producer_key, self._bls_key, self._name ))
        time.sleep( 30 )
        self._is_found = False
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"disableprods","json":"true"})).text)
    ...
    def delegateProducer( self ):
        cons_staked = 42100
        cmd_exec(Config.clfuture_path + '''  system  delegatecons  futureio  %s "%d FGAS" -p futureio@active  ''' % ( self._name, cons_staked ))
        time.sleep( 30 )
        self._is_found = False
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"producers","json":"true"})).text)
        for i in range(0,len(j["rows"])):
            if self._name == j["rows"][i]["owner"]:
                self.assertEqual(self._producer_key, j["rows"][i]["producer_key"])
                self.assertEqual(self._bls_key, j["rows"][i]["bls_key"])
    ...
    def undelegateProducer( self ):
        cmd_exec(Config.clfuture_path + '''  system  undelegatecons  futureio  %s -p futureio@active  ''' % ( self._name ))
        time.sleep( 30 )
        self._is_found = False
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"producers","json":"true"})).text)
        for i in range(0,len(j["rows"])):
