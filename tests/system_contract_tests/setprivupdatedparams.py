...
class SetPrivUpdatedPrams(unittest.TestCase):
    ...
    def setInitSetPrivUpdatedPrams( self ):
        cmd_exec(Config.clfuture_path + '''push action futureio  setpriv  '["%s", "%d"]' -f -p futureio ''' % ( self.name, self.privileged))
        cmd_exec(Config.clfuture_path + '''push action futureio  setupdateabled  '["%s", "%d"]' -f -p futureio ''' % ( self.name, self.updateable))

    # 测试设置的参数是否正确
    def test_SetPrivUpdatedPrams( self ):
        cmd_exec(Config.clfuture_path + '''push action futureio  setpriv  '["%s", "%d"]' -p futureio ''' % ( self.name, not self.privileged))
        cmd_exec(Config.clfuture_path + '''push action futureio  setupdateabled  '["%s", "%d"]' -p futureio ''' % ( self.name, not self.updateable))
        sleep(20)
        j = json.loads(requests.get(Config.get_account_info_url,data = json.dumps({"account_name":self.name})).text)
