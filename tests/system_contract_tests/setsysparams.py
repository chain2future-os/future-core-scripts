...
class SetsysparamsTest(unittest.TestCase):
    system = '''push action futureio setsysparams '{"params":{"chain_type": "0", "max_ram_size":"32212254720",        "min_activated_stake":420000000,"min_committee_member_number":4,        "block_reward_vec":[{"consensus_period":10,"reward":"10000"},{"consensus_period":2,"reward":"2000"}],        "max_resources_number":10000, "newaccount_fee":2000, "chain_name":"11", "worldstate_interval":1000,"resource_fee":35068,"table_extension":[[1,"10000"], [2, "12"], [3, "1"], [4, "true"], [5, "50"], [6, "2"]]}}' -p futureio"'''
    max_ram_size = 0
    ...
    def getInitsysparams( self ):
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"global"})).text)
