...
class Multichain(unittest.TestCase):
    name = "tester.11"
    pk = "FUT7aBDriMhNuHaySQknDGS4auB8RXAE1oQp3ZE9wo51PrUYauox1"
    ...
    account_pk_list = [
        ...
        "FUT6BThE2vci6Jqq5YyfYCy8mHMdk14Dfx9n5RWfkL8jcMDW7PJ5b",
        "FUT5kNr7rCgkkkyqXXZh51ZAHm8QbvhhX1KHH6wT4zzVMi7w8miZz",
        "FUT7bggm3iyTDmZMHfsnZ96N8wQnGmam6x97uuTYtxgSLWR3zyCix",
        "FUT8YCCmCEAM4GGqhSwQTB4SwP3J4sJTs721cAjZ6AXHr55qbig29",
    ]
    ...
    def clear_sidechain( self ):
        for chain in Config.chain_name:
            execcommand = Config.clfuture_path + 'push action futureio clearchain \'{"chain_name":"%s", "users_only":"0"}\' -p futureio@active' % (chain)
            cmd_exec( execcommand )
        sleep(10)
        ...
        for chain in Config.chain_name:
            for i in range(5):
                ...
                if "account_name" in j:
                    ...
                    execcommand = Config.clfuture_path + 'create account futureio %s %s %s ' % (account_name, self.account_pk_list[i], self.account_pk_list[i])
                    cmd_exec( execcommand )
                    cmd_exec( Config.clfuture_path + 'wallet import --private-key %s' % ( self.account_sk_list[i]))
    ...
    def reg_sidechain_producer( self ):
        for chain in Config.chain_name:
            for i in range(5):
                account_name = chain + self.accounts[i]
                execcommand = Config.clfuture_path + 'transfer futureio %s "10.0000 FGAS"' %account_name
                cmd_exec( execcommand )
        sleep(20)
        for chain in Config.chain_name:
            for i in range(5):
                ...
                for location in Config.chain_name:
                    execcommand = Config.clfuture_path + 'system empoweruser %s %s %s %s 1' % (account_name, location, self.account_pk_list[i], self.account_pk_list[i])
                    cmd_exec( execcommand )
        sleep(20)
        for chain in Config.chain_name:
            for i in range(5):
                account_name = chain + self.accounts[i]
                execcommand = Config.clfuture_path + 'system regproducer %s %s %s %s https://user.%s.com %s -p futureio@active -u' % (account_name, self.pk_list[i], self.bls_pk_list[i], account_name, account_name, chain)
                cmd_exec( execcommand )
        sleep(20)
        for chain in Config.chain_name:
            for i in range(5):
                account_name = chain + self.accounts[i]
                j = json.loads(requests.get(Config.get_table_url, data = json.dumps({"code":"futureio","scope":chain,"table":"producers","json":"true","table_key":account_name})).text)
                if len(j["rows"]) ==  0:
                    execcommand = Config.clfuture_path + 'system delegatecons futureio %s "42000.0000 FGAS" -p futureio@active' %account_name
                    cmd_exec( execcommand )
        sleep(10)
    ...
    def unreg_sidechain_producer( self ):
        for chain in Config.chain_name:
            for i in range(5):
                account_name = chain + self.accounts[i]
                execcommand = Config.clfuture_path + 'system undelegatecons futureio %s -p futureio@active' %account_name
                cmd_exec( execcommand )
        sleep(15)
        for chain in Config.chain_name:
            for i in range(5):
                account_name = chain + self.accounts[i]
                execcommand = Config.clfuture_path + 'push action futureio claimrewards \'{"producer":"%s"}\' -p %s@active' % (account_name, account_name)
                cmd_exec( execcommand )
        sleep(10)

    def empower_user( self ):
        for a in Config.chain_name:
            execcommand = Config.clfuture_path + 'system empoweruser %s %s %s %s 1' % (self.name, a, self.pk, self.pk)
            cmd_exec( execcommand )
        sleep(self.multi_chain_trx_time)

    def resource_lease( self ):
        execcommand = Config.clfuture_path + 'system resourcelease futureio %s 80 1 %s' % (self.name, Config.chain_name[0])
        cmd_exec( execcommand )
        sleep(self.multi_chain_trx_time)

    def transfer_to_bank( self, src = "master", chain_index = 0):
        if src == "master":
            execcommand = Config.clfuture_path + 'transfer %s.111 futio.bank "10.0000 FGAS" %s' % (Config.chain_name[chain_index], Config.chain_name[chain_index])
            cmd_exec( execcommand )
        else:
            execcommand = Config.clfuture_path + '--url %s transfer %s.111 futio.bank "10.0000 FGAS" futureio' % (Config.node_url[chain_index], Config.chain_name[chain_index])
            cmd_exec( execcommand )
        #wait enough time for lws sync
        sleep(self.multi_chain_trx_time)

    def move_producer( self, prod_base, producer_index, src_chain_index, dest_chain_index):
        execcommand = Config.clfuture_path + 'push action futureio moveprod \'{"producer":"%s", "producerkey":%s, "blskey":%s, "from_disable":"0", "from_chain":"%s", "to_disable":"0", "to_chain":"%s"}\' -p futureio@active' % ((prod_base + self.accounts[producer_index]), self.pk_list[producer_index], self.bls_pk_list[producer_index], Config.chain_name[src_chain_index], Config.chain_name[dest_chain_index])
        cmd_exec( execcommand )
        sleep(self.multi_chain_trx_time)

    def enable_schedule( self ):
        execcommand = Config.clfuture_path + 'push action futureio setsched \'{"is_enabled":"1", "sched_period":"60", "expire_time":"15"}\' -p futureio@active'
        cmd_exec( execcommand )
        sleep(10)

    def disable_schedule( self ):
        execcommand = Config.clfuture_path + 'push action futureio setsched \'{"is_enabled":"0", "sched_period":"60", "expire_time":"15"}\' -p futureio@active'
        cmd_exec( execcommand )
        sleep(10)
    ...
    def test_lwc_sync( self ):
        ...
        j = json.loads(requests.get(Config.get_table_url, data = json.dumps({"code":"futureio","scope":Config.chain_name[0],"table":"producers","json":"true","table_key":producer})).text)
        self.assertEqual(len(j["rows"]), 0)
        j = json.loads(requests.get(Config.get_table_url, data = json.dumps({"code":"futureio","scope":Config.chain_name[1],"table":"producers","json":"true","table_key":producer})).text)
        self.assertEqual(len(j["rows"]), 1)
        get_sidechain1_table_url = Config.node_url[0] + 'v1/chain/get_table_records'
        j = json.loads(requests.get(get_sidechain1_table_url, data = json.dumps({"code":"futureio","scope":"futureio","table":"producers","json":"true","table_key":producer})).text)
        self.assertEqual(len(j["rows"]), 0)
        get_sidechain2_table_url = Config.node_url[1] + 'v1/chain/get_table_records'
        j = json.loads(requests.get(get_sidechain2_table_url, data = json.dumps({"code":"futureio","scope":"futureio","table":"producers","json":"true","table_key":producer})).text)
        self.assertEqual(len(j["rows"]), 1)
