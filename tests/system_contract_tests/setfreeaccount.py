...
class SetFreeAccount(unittest.TestCase):
    ...
    def getInitFreeAccountNum( self ):
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"freeacc","json":"true"})).text)
    ...
    def setInitFreeAccountNum( self ):
        cmd_exec(Config.clfuture_path + '''push action futureio  setfreeacc  '["%s", "%d"]' -f -p futureio ''' % ( self.name, self.free_num))

    # 测试设置的参数是否正确
    def testSetFreeAccount( self ):
        cmd_exec(Config.clfuture_path + '''push action futureio  setfreeacc  '["%s", "%d"]' -p futureio ''' % ( self.name, self.free_num+1))
        sleep(20)
        freeaccount = 0
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"freeacc","json":"true"})).text)
        for i in range(0,len(j["rows"])):
