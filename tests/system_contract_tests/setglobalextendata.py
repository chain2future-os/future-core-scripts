...
class SetGlobalExtenData(unittest.TestCase):
    ...
    def getInitglobalparams( self ):
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"global","json":"true"})).text)
    ...
    def setInitglobalparams( self ):
        for key in self.d:
            execcommand = Config.clfuture_path + '''push action futureio  setglobalextendata  '["%d", "%d"]' -f -p futureio ''' % ( key, self.d[key])
            print( execcommand )
    ...
    def test_setglobalextendata( self ):
        for key in self.d:
            execcommand = Config.clfuture_path + '''push action futureio  setglobalextendata  '["%d", "%d"]' -p futureio ''' % ( key, self.d[key] + 1 )
            print( execcommand )
            subprocess.call( execcommand, shell=True )
        #time.sleep(15)
        j = json.loads(requests.get(Config.get_table_url,data = json.dumps({"code":"futureio","scope":"futureio","table":"global","json":"true"})).text)
