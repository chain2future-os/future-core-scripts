...
case "$1" in
  "e") #vote
	...
	do
		printf "times : "$j"\n"
		res=$($clDir/clfuture --wallet-url http://127.0.0.1:6666 push action futio.rand query '' -p ${ult_accountLst[$i]})
		...
		do
			...
			if [ -n $random ]; then
				...
				if [ "$existing" = "false" ]; then
					...
					$clDir/clfuture --wallet-url http://127.0.0.1:6666 push action futio.rand vote '["'"${data1[1]}"'", "'"${bcknum}"'"]' -p ${ult_accountLst[$i]}
				fi
				len=${#voted_data[*]}
