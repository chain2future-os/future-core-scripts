#!/bin/bash
#
# futureio-tn_roll is used to have all of the instances of the FUTURE daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: futureio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./futureio-tn_roll.sh is sufficient.
#

if [ -z "$FUTUREIO_HOME" ]; then
    echo FUTUREIO_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in futured futureiod nodfuture; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for futured or futureiod or nodfuture
    exit 1
fi
...
if [ $DD = "all" ]; then
    for FUTUREIO_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $FUTUREIO_HOME/scripts/futureio-tn_up.sh $*
    done
else
    bash $FUTUREIO_HOME/scripts/futureio-tn_up.sh $*
fi
unset FUTUREIO_RESTART_DATA_DIR
